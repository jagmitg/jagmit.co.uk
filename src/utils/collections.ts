...
function normalizeSlug(id: string): string {
    const normalized = id.replace(/^(content|repo)\//, '')
    return normalized
}
...
export async function getSortedPosts(): Promise<Post[]> {
    ...
    const repoPosts: RepoPost[] = allRepoPosts
        .map((post) => ({
            ...post,
            slug: normalizeSlug(post.id)
        }))
...
export function sortByLastEditDate(
    ...
    const dateA = new Date(a.data.last_edit_date || a.data.creation_date).getTime()
    const dateB = new Date(b.data.last_edit_date || b.data.creation_date).getTime()
    return isDesc ? dateB - dateA : dateA - dateB
}
...
export function sortByViewCount(
    ...
    return isDesc ? b.data.view_count - a.data.view_count : a.data.view_count - b.data.view_count
}

export function getSortFunction(sortType: string) {
    return (a: CollectionEntry<'stackoverflow'>, b: CollectionEntry<'stackoverflow'>): number => {
        switch (sortType) {
